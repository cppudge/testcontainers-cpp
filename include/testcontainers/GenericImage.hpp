...
public:
    ...
    /// a purely local check (`GET /images/{ref}/json`), no registry is
    /// contacted. Useful to skip an expensive `GenericBuildableImage::build()`
    /// when a previous run already produced the image:
    /// ```cpp
