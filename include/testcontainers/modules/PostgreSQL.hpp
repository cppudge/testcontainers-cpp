...
/// returns a `PostgreSQLContainer` whose getters hand out ready-to-paste
/// connection strings.
///
...
public:
    ...
    /// `PostgreSQLContainer::port()` (the mapped host port) instead.
    ...
    /// defaults to "latest").
    PostgreSQLImage& with_image(const std::string& reference);

    /// Superuser name the database is initialized with (POSTGRES_USER).
    /// Default "test".
    PostgreSQLImage& with_username(std::string username);
    ...
    PostgreSQLImage& with_password(std::string password);

    /// Name of the database created at first boot (POSTGRES_DB). Default "test".
    PostgreSQLImage& with_database(std::string database);
    ...
    PostgreSQLImage& with_init_script(std::filesystem::path host_path);
    ...
    PostgreSQLImage& with_init_script(const std::string& name, std::string content);
    ...
    PostgreSQLImage& with_config_option(std::string key, std::string value);
    ...
    /// duplicates set here.
    PostgreSQLImage& with_env(std::string key, std::string value);

    PostgreSQLImage& with_label(std::string key, std::string value);

    /// Join a user-defined network; peers resolve this container by
    /// name/alias at `<alias>:5432` (kPort, not the mapped host port).
    PostgreSQLImage& with_network(std::string network);
    PostgreSQLImage& with_network(const Network& network);
    PostgreSQLImage& with_network_alias(std::string alias);
    ...
    /// its data intact.
    PostgreSQLImage& with_reuse(bool reuse = true);
    ...
    PostgreSQLImage& with_wait(WaitFor wait);
    ...
    /// count against it.
    PostgreSQLImage& with_startup_timeout(std::chrono::milliseconds timeout);

    /// Retry the whole create→start→wait sequence up to `n` times.
    PostgreSQLImage& with_startup_attempts(int n);
    ...
    /// auth) instead.
    PostgreSQLImage& with_customizer(std::function<void(GenericImage&)> customize);
    ...
    /// plain GenericImage: the drop-down escape hatch when you need a raw core
    /// `Container` (or run-level tweaks) instead of a PostgreSQLContainer.
...
public:
    ...
    /// `<alias>:5432` (PostgreSQLImage::kPort) instead — build that DSN
...
private:
    ...
    PostgreSQLContainer(Container container, std::string username, std::string password,
                        std::string database)
        : container_(std::move(container)), username_(std::move(username)),
          ...
          port_(container_.get_host_port(tcp(PostgreSQLImage::kPort))) {}
