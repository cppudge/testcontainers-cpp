...
/// returns a `MySQLContainer` handle that owns the running container and
...
public:
    ...
    /// uses `MySQLContainer::port()` (the mapped host port) instead.
    ...
    /// `Container` (or run-level tweaks) instead of a MySQLContainer. Throws
...
private:
    ...
    MySQLContainer(Container container, std::string username, std::string password,
                   std::string database)
        : container_(std::move(container)), username_(std::move(username)),
          ...
          port_(container_.get_host_port(tcp(MySQLImage::kPort))) {}
