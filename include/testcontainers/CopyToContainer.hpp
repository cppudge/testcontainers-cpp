...
public:
    /// Copy a host file to `container_target` (an absolute path in the
    /// container, e.g. "/tmp/foo.txt"). The file is read when the copy runs.
    static CopyToContainer host_file(std::string host_path, std::string container_target) {
        CopyToContainer c;
        c.is_file_ = true;
    ...
    const std::string& host_path() const noexcept { return host_path_; }
...
private:
    bool is_file_ = false;
    std::string host_path_; ///< set when is_file_
