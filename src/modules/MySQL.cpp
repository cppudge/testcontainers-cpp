...
MySQLImage::MySQLImage() : image_(GenericImage::from_reference(std::string(kDefaultImage))) {
...
MySQLImage& MySQLImage::with_image(const std::string& reference) {
    image_.with_image(reference);
    return *this;
}

MySQLImage& MySQLImage::with_username(std::string username) {
...
MySQLImage& MySQLImage::with_init_script(std::filesystem::path host_path) {
    detail::add_init_script(opts_, std::move(host_path));
    return *this;
}

MySQLImage& MySQLImage::with_init_script(const std::string& name, std::string content) {
    detail::add_init_script(opts_, name, std::move(content));
    return *this;
}

MySQLImage& MySQLImage::with_config_file(std::filesystem::path host_cnf) {
    detail::add_config_file(opts_, std::move(host_cnf));
    return *this;
}

// Out of line so the header needs no Network definition.
MySQLImage& MySQLImage::with_network(const Network& network) {
    image_.with_network(network);
    return *this;
}

GenericImage MySQLImage::to_generic() const {
    return detail::render(detail::mysql_flavor(), image_, opts_);
}

MySQLContainer MySQLImage::start() const {
    return MySQLContainer(to_generic().start(), opts_.username, opts_.password, opts_.database);
}

std::string MySQLContainer::connection_string() const {
