...
//   ErrorModel.GuardedParsersWrapInvalidJson - every parse entry point (inspect / container+network+volume list / volume prune / version / exec) turns an HTML-through-200 body into a DockerError naming its operation, never a raw json exception.
...
TEST(ErrorModel, GuardedParsersWrapInvalidJson) {
    ...
    expect_wrapped([&] { docker::parse_volume_list(html); }, "list_volumes");
    expect_wrapped([&] { docker::parse_volume_prune(html); }, "prune_volumes");
