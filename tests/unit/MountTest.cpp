...
//   Mount.ReadOnlySetter - read_only() flips the read-only flag and defaults to true.
...
TEST(Mount, SettersChainOnRvalue) {
    const Mount m = Mount::tmpfs("/cache").with_tmpfs_size(1024).read_only();
    EXPECT_TRUE(m.is_read_only());
    ASSERT_TRUE(m.tmpfs_size().has_value());
...
TEST(Mount, Copyable) {
    const Mount original = Mount::bind("/host", "/data").read_only();
