...
- **CI analysis follow-ups** (from the 2026-07-05 review of the analysis gates) —
  `TC_WERROR` + unpinned runner compilers means occasional `-Wno-error=<warning>`
  maintenance when a runner image bumps gcc (ci.yml) or clang (the CodeQL traced build
  compiles with clang since 2026-07-10, closing the former no-clang-compile-job gap;
  it triggers only on main pushes / PRs, so a clang-only breakage surfaces later than
  the per-push gcc jobs do). The former consumer-smoke and macOS gaps are covered by the
  `conan create` job (test_package consumes the package; macOS runs the unit suite there).
