...
- **Packaging residuals** — the installed `testcontainersConfig.cmake` resolves dependencies
  by Conan's target names (boost::boost, Libssh2::libssh2), so the non-Conan consumption
  ...
  `test_package/` is outside the clang-tidy job's globs. For ConanCenter specifically:
  recipe restructure (conandata.yml + source() from a release tarball, no exports_sources),
  dropping the forced boost/libarchive options, and a validate() compiler-version floor.
